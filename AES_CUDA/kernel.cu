...
__device__ static const BYTE d_aes_sbox[16][16] = SBOX;

__device__ static const BYTE aes_invsbox[16][16] = {
...
__global__ void cuda_aes_encrypt(BYTE *in, BYTE *out, int n)
{
	...
	{
		//Call the encrypt function on the current 16-byte block
		aes_encrypt(&in[i], &out[i], d_keySchedule, 128);
	}
}
...
__device__ void SubBytes(BYTE state[4][4])
{
	state[0][0] = d_aes_sbox[state[0][0] >> 4][state[0][0] & 0x0F];
	state[0][1] = d_aes_sbox[state[0][1] >> 4][state[0][1] & 0x0F];
	state[0][2] = d_aes_sbox[state[0][2] >> 4][state[0][2] & 0x0F];
	state[0][3] = d_aes_sbox[state[0][3] >> 4][state[0][3] & 0x0F];
	state[1][0] = d_aes_sbox[state[1][0] >> 4][state[1][0] & 0x0F];
	state[1][1] = d_aes_sbox[state[1][1] >> 4][state[1][1] & 0x0F];
	state[1][2] = d_aes_sbox[state[1][2] >> 4][state[1][2] & 0x0F];
	state[1][3] = d_aes_sbox[state[1][3] >> 4][state[1][3] & 0x0F];
	state[2][0] = d_aes_sbox[state[2][0] >> 4][state[2][0] & 0x0F];
	state[2][1] = d_aes_sbox[state[2][1] >> 4][state[2][1] & 0x0F];
	state[2][2] = d_aes_sbox[state[2][2] >> 4][state[2][2] & 0x0F];
	state[2][3] = d_aes_sbox[state[2][3] >> 4][state[2][3] & 0x0F];
	state[3][0] = d_aes_sbox[state[3][0] >> 4][state[3][0] & 0x0F];
	state[3][1] = d_aes_sbox[state[3][1] >> 4][state[3][1] & 0x0F];
